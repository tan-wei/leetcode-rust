...
mod tests {
    ...
    #[test]
    #[ignore]
    fn test_2382_example_1() {
        let nums = vec![1, 2, 5, 6, 1];
        let remove_queries = vec![0, 3, 2, 4, 1];

        let result = vec![14, 7, 2, 2, 0];

        assert_eq!(Solution::maximum_segment_sum(nums, remove_queries), result);
    }

    #[test]
    #[ignore]
    fn test_2382_example_2() {
        let nums = vec![3, 2, 11, 1];
        let remove_queries = vec![3, 2, 1, 0];

        let result = vec![16, 5, 3, 0];
