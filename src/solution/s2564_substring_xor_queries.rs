...
mod tests {
    ...
    #[test]
    #[ignore]
    fn test_2564_example_1() {
        let s = "0101".to_string();
        let queries = vec![vec![0, 5], vec![1, 2]];

        let result = vec![vec![0, 2], vec![2, 3]];

        assert_eq!(Solution::substring_xor_queries(s, queries), result);
    }

    #[test]
    #[ignore]
    fn test_2564_example_2() {
        let s = "101101".to_string();
        let queries = vec![vec![12, 8]];
