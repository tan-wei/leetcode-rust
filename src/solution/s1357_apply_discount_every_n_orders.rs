...
struct Cashier {
    n: i32,
    discount: i32,
    products: std::collections::HashMap<i32, i32>,
    count: i32,
}
...
mod tests {
    ...
    fn test_1357_example_1() {
        ...
        assert_f64_near!(cashier.get_bill(vec![1, 2], vec![1, 2]), 500.0);
        assert_f64_near!(cashier.get_bill(vec![3, 7], vec![10, 10]), 4000.0);
        assert_f64_near!(
            cashier.get_bill(vec![1, 2, 3, 4, 5, 6, 7], vec![1, 1, 1, 1, 1, 1, 1]),
            800.0
        );
        assert_f64_near!(cashier.get_bill(vec![4], vec![10]), 4000.0);
        assert_f64_near!(cashier.get_bill(vec![7, 3], vec![10, 10]), 4000.0);
        assert_f64_near!(
            cashier.get_bill(vec![7, 5, 3, 1, 6, 4, 2], vec![10, 10, 10, 9, 9, 9, 7]),
            7350.0
        );
        assert_f64_near!(cashier.get_bill(vec![2, 3, 5], vec![5, 3, 2]), 2500.0);
    }
}
