...
pub struct Twitter {
    ...
    follows: std::collections::HashSet<(User, User)>,
}
...
impl Twitter {
    ...
    fn post_tweet(&mut self, user_id: User, tweet_id: Tweet) {
        self.tweets.push((tweet_id, user_id));
    }
    fn get_news_feed(&self, user_id: User) -> Vec<Tweet> {
        self.tweets
            .iter()
            .rev()
            .filter(|&&(_, author)| user_id == author || self.follows.contains(&(user_id, author)))
            .map(|&(tweet, _)| tweet)
            .take(10)
            .collect()
    }
    pub fn follow(&mut self, follower_id: User, followee_id: User) {
        self.follows.insert((follower_id, followee_id));
    }

    pub fn unfollow(&mut self, follower_id: User, followee_id: User) {
