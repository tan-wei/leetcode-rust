    ...
    pub fn restore_matrix(row_sum: Vec<i32>, col_sum: Vec<i32>) -> Vec<Vec<i32>> {
        ...
        (0..n)
            .map(|row_index| {
                (0..m)
                    .map(|col_index| {
                        let val = std::cmp::min(row_sum[row_index], col_sum[col_index]);
                        row_sum[row_index] -= val;
                        col_sum[col_index] -= val;
                        val
                    })
                    .collect::<Vec<i32>>()
            })
...
mod tests {
    ...
    #[test]
    #[ignore]
    fn test_1605_example_1() {
        let row_sum = vec![3, 8];
        let col_sum = vec![4, 7];

        let result = vec![vec![3, 0], vec![1, 7]];

        assert_eq!(Solution::restore_matrix(row_sum, col_sum), result);
    }

    #[test]
    #[ignore]
    fn test_1605_example_2() {
        let row_sum = vec![5, 7, 10];
        let col_sum = vec![8, 6, 8];

        let result = vec![vec![0, 5, 0], vec![6, 1, 0], vec![2, 0, 8]];
