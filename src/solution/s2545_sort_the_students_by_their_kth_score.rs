...
mod tests {
    ...
    #[test]
    #[ignore]
    fn test_2545_example_1() {
        let score = vec![vec![10, 6, 9, 1], vec![7, 5, 11, 2], vec![4, 8, 3, 15]];
        let k = 2;

        let result = vec![vec![7, 5, 11, 2], vec![10, 6, 9, 1], vec![4, 8, 3, 15]];

        assert_eq!(Solution::sort_the_students(score, k), result);
    }

    #[test]
    #[ignore]
    fn test_2545_example_2() {
        let score = vec![vec![3, 4], vec![5, 6]];
        let k = 0;

        let result = vec![vec![5, 6], vec![3, 4]];
