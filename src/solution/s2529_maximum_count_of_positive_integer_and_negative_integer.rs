...
impl Solution {
    pub fn maximum_count(nums: Vec<i32>) -> i32 {
        nums.iter()
            .filter(|x| **x > 0)
            .count()
            .max(nums.iter().filter(|x| **x < 0).count()) as _
    }
}
...
mod tests {
    ...
    #[test]
    fn test_2529_example_1() {
        let nums = vec![-2, -1, -1, 1, 2, 3];

        let result = 3;

        assert_eq!(Solution::maximum_count(nums), result);
    }

    #[test]
    fn test_2529_example_2() {
