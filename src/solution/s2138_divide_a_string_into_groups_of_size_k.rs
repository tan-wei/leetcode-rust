...
impl Solution {
    pub fn divide_string(s: String, k: i32, fill: char) -> Vec<String> {
        ...
        s.as_bytes()
            .chunks(k)
            .map(|group| {
                let mut g_string = String::from_utf8(group.to_vec()).unwrap();
                while g_string.len() < k {
                    g_string.push(fill);
                }
                g_string
            })
...
mod tests {
    ...
    #[test]
    fn test_2138_example_1() {
        let s = "abcdefghi".to_string();
        let k = 3;
        let fill = 'x';

        let result = vec_string!["abc", "def", "ghi"];

        assert_eq!(Solution::divide_string(s, k, fill), result);
    }

    #[test]
    fn test_2138_example_2() {
        let s = "abcdefghij".to_string();
        let k = 3;
        let fill = 'x';

        let result = vec_string!["abc", "def", "ghi", "jxx"];
