...
impl Solution {
    pub fn find_relative_ranks(score: Vec<i32>) -> Vec<String> {
        ...
        let mut score_rank = std::collections::HashMap::new();

        for i in 0..score_sorted.len() {
            match i {
                0 => score_rank.insert(score_sorted[i], "Gold Medal".to_string()),
                1 => score_rank.insert(score_sorted[i], "Silver Medal".to_string()),
                2 => score_rank.insert(score_sorted[i], "Bronze Medal".to_string()),
                _ => score_rank.insert(score_sorted[i], (i + 1).to_string()),
            };
        }

        score
            .iter()
            .map(|x| score_rank.get(x).unwrap().to_string())
            .collect()
    }
}
...
mod tests {
    ...
    #[test]
    fn test_0506_example_1() {
        let score = vec![5, 4, 3, 2, 1];
        let result = vec_string!["Gold Medal", "Silver Medal", "Bronze Medal", "4", "5"];

        assert_eq!(Solution::find_relative_ranks(score), result);
    }

    #[test]
    fn test_0506_example_2() {
        let score = vec![10, 3, 8, 9, 4];
        let result = vec_string!["Gold Medal", "5", "Bronze Medal", "Silver Medal", "4"];
