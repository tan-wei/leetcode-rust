...
impl Solution {
    pub fn average_waiting_time(customers: Vec<Vec<i32>>) -> f64 {
        ...
        for customer in &customers {
            let (arrival, load) = (customer[0] as i64, customer[1] as i64);
            if arrival > total_time {
                total_time = arrival + load
            } else {
                total_time += load
            }
        ...
        wait_time as f64 / (customers.len() as i64) as f64
    }
}
...
mod tests {
    ...
    #[test]
    fn test_1701_example_1() {
        let customers = vec![vec![1, 2], vec![2, 5], vec![4, 3]];

        let result = 5.00000;

        assert_f64_near!(Solution::average_waiting_time(customers), result);
    }

    #[test]
    fn test_1701_example_2() {
