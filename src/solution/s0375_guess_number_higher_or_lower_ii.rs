...
impl Solution {
    pub fn get_money_amount(n: i32) -> i32 {
        ...
        return Self::dfs_helper(1, n - 1, &mut dp);
    }

    pub fn dfs_helper(start: usize, end: usize, dp: &mut Vec<Vec<i32>>) -> i32 {
        if start >= end {
            return 0;
        }
        ...
        for i in start..(end + 1) {
            let amt = i as i32;
            let tmp =
                amt + Self::dfs_helper(i + 1, end, dp).max(Self::dfs_helper(start, i - 1, dp));
            res = res.min(tmp);
        }
        dp[start][end] = res;
        return res;
...
mod tests {
    ...
    #[test]
    fn test_0375_example_1() {
        let n = 10;
        let result = 16;

        assert_eq!(Solution::get_money_amount(n), result);
    }

    #[test]
    fn test_0375_example_2() {
        let n = 1;
        let result = 0;
