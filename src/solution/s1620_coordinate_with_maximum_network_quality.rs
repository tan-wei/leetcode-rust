...
impl Solution {
    // Credit: https://leetcode.com/problems/coordinate-with-maximum-network-quality/solutions/3181867/just-a-runnable-solution/
...
mod tests {
    ...
    #[test]
    fn test_1620_example_1() {
        let towers = vec![vec![1, 2, 5], vec![2, 1, 7], vec![3, 1, 9]];
        let radius = 2;

        let result = vec![2, 1];

        assert_eq!(Solution::best_coordinate(towers, radius), result);
    }

    #[test]
    fn test_1620_example_2() {
        let towers = vec![vec![23, 11, 21]];
        let radius = 9;

        let result = vec![23, 11];

        assert_eq!(Solution::best_coordinate(towers, radius), result);
    }

    #[test]
    fn test_1620_example_3() {
